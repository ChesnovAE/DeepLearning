...
class DeepQNetwork(nn.Module):
    ...
    def forward(self, observations):
        ...
        observations = observations.view(-1, 1, 185, 95)
        ...
        observations = observations.view(-1, 128*19*8)
        observations = F.relu(self.fc1(observations))

        actions = self.fc2(observations)
...
class Agent(object):
    ...
    def storeTransition(self, state, action, reward, state_):
        if self.memCntr < self.memSize:
            ...
            self.memory[self.memCntr % self.memSize] = [state,
                                                        action,
                                                        reward,
                                                        state_]
        self.memCntr += 1
    ...
    def learn(self, batch_size):
        ...
        Qpred = self.Q_eval.forward(list(memory[:, 0][:])).to(self.Q_eval.device) # 0 - state
        Qnext = self.Q_next.forward(list(memory[:, 3][:])).to(self.Q_next.device) # 3 - state_
